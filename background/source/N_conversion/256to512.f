...
C     Read in free functions from dat files. 
      open(unit=10, file='256/Delta.out', status='old')
      read(10,*) Delta
      close(10)
      open(unit=10, file='256/fc.out', status='old')
      do j=1,ny
         read(10,*) fc(j)
      end do
      close(10)
      open(unit=10, file='256/psic.out', status='old')
      do j=1,ny
         read(10,*) psic(j)
      end do
      close(10)    
      open(unit=10, file='256/Up.out', status='old')
      do j=1,ny
         read(10,*) Up(j)
      end do
      close(10)
